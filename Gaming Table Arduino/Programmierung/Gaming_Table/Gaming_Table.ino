...
unsigned int select = 0;  // select muss zum start 0 sein, bis jetzt select 0 bis 6 belegt!!
int select_input = 12;
int x_one;
int y_one;
int x_two;
...
int courser_on_STOP_AND_GO = false;
/************************************/
...
void start_program() {
  if (standby_Timer > st) {
    select = "A";  // select = "A" wird nirgendwo verwendet, verhindert nur das ungewollte aussteigen aus dem standby Modus
    runlight();
    reset();
  }
  if (select == 0) {
    ...
    if (select == 0) {
      ...
      if (y_one <= 150 || y_two <= 150 || y_three <= 150 || y_four <= 150 || courser_on_TIC_TAC_TOE) {
        ...
        for (int i = 1701; i < 2187; i++) {
          if (i == 1701 || i == 1705 || i == 1749 || i == 1753 || i == 1797 || i == 1801 || i >= 1891 && i <= 1899 || i >= 1938 && i <= 1947 || i >= 1987 && i <= 1995 || i == 2085 || i == 2089 || i == 2133 || i == 2137 || i == 2181 || i == 2185 || i >= 2034 && i <= 2046 || i >= 1842 && i <= 1854) {
            led.setPixelColor(i, led.Color(0, 0, 0));
          }
        }
        led.show();
      ...
      //STOP AND GO//
      if (y_one <= 200 && x_one >= 800 || y_two <= 200 && x_two >= 800 || y_three <= 200 && x_three >= 800 || y_four <= 200 && x_four >= 800 || courser_on_STOP_AND_GO) {
        courser_on_TIC_TAC_TOE = false;
        courser_on_RACE_GAME = false;
        courser_on_COIN_GAME = false;
        courser_on_JUMP_AND_RUN = false;
        courser_on_STOP_AND_GO = true;
        for (int i = 1433; i < 2060; i++) {
          if (i == 1532 || i == 1484 || i == 1435 || i == 1434 || i == 1481 || i == 1529 || i == 1577 || i == 1625 || i == 1673 || i == 1721 || i == 1769 || i == 1817 || i == 1866 || i == 1819 || i == 1820 || i == 1869 || i == 1822 || i == 1870 || i == 1918 || i == 1965 || i == 2012 || i == 2059 || i == 2058 || i == 2057 || i == 2056 || i == 2055 || i == 2006 || i == 1957 || i == 1908 || i == 1860 || i == 1812 || i == 1861 || i == 1814 || i == 1815 || i == 1864) {
...
void game_ONE_LED_RACE() {
  if (select == 1) {
    ...
    if (run == 1) {
      easter_counter = 0;
      //Festlegen der Geschwindikeit der Spieler
...
void dev_made_easy() {
  ...
  if (god_mode.equals("48")) {
    select = 0;  //0 , startet den Home Bildschirm
    for (int i = 0; i < MAXLED; i++) { led.setPixelColor(i, led.Color(0, 0, 0)); }
  }
  if (god_mode.equals("49")) { select = 1; }             //1 , startet spiel eins RACE GAME
  if (god_mode.equals("50")) { select = 2; }             //2 , startet spiel zwei TIC TAC TOE
  if (god_mode.equals("51")) { select = 3; }             //3 , startet spiel drei COIN COLLECT
  if (god_mode.equals("52")) { select = 4; }             //4 , startet spiel vier JUMP AND RUN
  if (god_mode.equals("53")) { select = 5; }             //5 , startet spiel fünf STOP AND GO
